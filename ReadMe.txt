...
【 框 架 工 具 】
 Python3 + Selenium3 + Flask + uWSGI + Nginx + MongoDB + Docker + Fabric + Gulp
...
1.使用 Python3 + Selenium3 :
（1）实现模拟用户操作浏览器进行测试
（2）通过动态修改和添加'<TestSuite>'类中的方法和属性，实现启用多线程同时执行多条测试用例
（3）通过修改'HTMLTestRunnerCN'文件，优化了测试报告的展示方式，并提供了每个测试用例的截图显示
